...
// Taken from qtbase/src/gui/image/qpixmap_win.cpp
enum HBitmapFormatSimple {
	HBitmapNoAlphaSimple,
	HBitmapPremultipliedAlphaSimple,
	HBitmapAlphaSimple
};

class SimpleEventFilter final : public QAbstractNativeEventFilter {
public:
	explicit SimpleEventFilter(not_null<SimpleWindow*> window);
...
private:
	...
	const not_null<SimpleWindow*> _window;
...
SimpleEventFilter::SimpleEventFilter(not_null<SimpleWindow*> window) : _window(window) {
}

bool SimpleEventFilter::nativeEventFilter(
		const QByteArray &eventType,
		void *message,
...
bool SimpleEventFilter::mainWindowEvent(
		HWND hWnd,
		UINT msg,
		WPARAM wParam,
		LPARAM lParam,
...
struct SimpleWindow::Private {
	explicit Private(not_null<SimpleWindow*> window) : filter(window) {
	}
...
SimpleWindow::BitmapPointer::BitmapPointer(HBITMAP value) : _value(value) {
}

SimpleWindow::BitmapPointer::BitmapPointer(BitmapPointer &&other)
: _value(base::take(other._value)) {
}

SimpleWindow::BitmapPointer &SimpleWindow::BitmapPointer::operator=(
		BitmapPointer &&other) {
...
SimpleWindow::BitmapPointer::~BitmapPointer() {
	reset();
}

HBITMAP SimpleWindow::BitmapPointer::get() const {
	return _value;
}

SimpleWindow::BitmapPointer::operator bool() const {
	return _value != nullptr;
}

void SimpleWindow::BitmapPointer::release() {
	_value = nullptr;
}

void SimpleWindow::BitmapPointer::reset(HBITMAP value) {
	if (_value != value) {
...
void SimpleWindow::setupPreviewPasscodeLock() {
...
void SimpleWindow::setupNativeWindowFrame() {
	validateWindowTheme(false, false);
}

void SimpleWindow::shadowsActivate() {
	_hasActiveFrame = true;
}

void SimpleWindow::shadowsDeactivate() {
	_hasActiveFrame = false;
}

void SimpleWindow::destroyedFromSystem() {
	Core::Quit();
}

bool SimpleWindow::setDwmThumbnail(QSize size) {
	...
	if (size.isEmpty()) {
		...
		const auto bitmap = qt_imageToWinHBITMAP(result, HBitmapNoAlphaSimple);
		if (!bitmap) {
			return false;
		}
...
bool SimpleWindow::setDwmPreview(QSize size, int radius) {
	Expects(radius >= 0);
	...
	if (size.isEmpty()) {
		...
		const auto bitmap = qt_imageToWinHBITMAP(
			result,
			(radius > 0) ? HBitmapPremultipliedAlphaSimple : HBitmapNoAlphaSimple);
		if (!bitmap) {
			return false;
		}
...
void SimpleWindow::validateDwmPreviewColors() {
...
void SimpleWindow::forceIconRefresh() {
...
void SimpleWindow::workmodeUpdated(Core::Settings::WorkMode mode) {
...
bool SimpleWindow::hasTabletView() const {
...
bool SimpleWindow::initGeometryFromSystem() {
...
void SimpleWindow::updateWindowIcon() {
	updateTaskbarAndIconCounters();
}

void SimpleWindow::unreadCounterChangedHook() {
	updateTaskbarAndIconCounters();
}

void SimpleWindow::updateTaskbarAndIconCounters() {
...
void SimpleWindow::validateWindowTheme(bool native, bool night) {
...
HWND SimpleWindow::psHwnd() const {
	return _hWnd;
}

void SimpleWindow::destroyCachedIcons() {
