...
void SimpleWindow::updateWindowIcon() {
	setWindowIcon(Window::CreateIcon(nullptr));
}

void SimpleWindow::updateUnityCounter() {
...
void SimpleWindow::updateGlobalMenuHook() {
	...
	updateIsActive();
	const auto logged = false;//(sessionController() != nullptr);
	const auto inactive = true;//!logged || controller().locked();
