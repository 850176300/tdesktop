...
@interface SimpleWindowObserver : NSObject {
}

- (id) init:(SimpleWindow::Private*)window;
...
@end // @interface SimpleWindowObserver
...
class SimpleWindow::Private {
public:
	explicit Private(not_null<SimpleWindow*> window);

	void setNativeWindow(NSWindow *window, NSView *view);
	void initTouchBar(NSWindow *window);
...
private:
	...
	SimpleWindowObserver *_observer = nullptr;
...
@implementation SimpleWindowObserver {
	SimpleWindow::Private *_private;
...
@end // @implementation SimpleWindowObserver
...
SimpleWindow::Private::Private(not_null<SimpleWindow*> window)
: _public(window)
, _observer([[SimpleWindowObserver alloc] init:this]) {
...
void SimpleWindow::Private::setWindowBadge(const QString &str) {
	@autoreleasepool {
...
void SimpleWindow::Private::setNativeWindow(NSWindow *window, NSView *view) {
...
void SimpleWindow::Private::initTouchBar(NSWindow *window) {
...
void SimpleWindow::Private::setMarkdownEnabledState(
		Ui::MarkdownEnabledState state) {
	_markdownState = state;
}

bool SimpleWindow::Private::clipboardHasText() {
...
void SimpleWindow::closeWithoutDestroy() {
...
void SimpleWindow::stateChangedHook(Qt::WindowState state) {
...
void SimpleWindow::hideAndDeactivate() {
	hide();
}
...
void SimpleWindow::updateDockCounter() {
...
void SimpleWindow::createGlobalMenu() {
	...
	auto main = psMainMenu.addMenu(u"Toome"_q);
	{
		auto callback = [=] {
			ensureWindowShown();
...
void SimpleWindow::updateGlobalMenuHook() {
	if (!positionInited()) {
		return;
	}
	...
	updateIsActive();
	const auto logged = false;
	const auto inactive = true;
	const auto support = false;
...
bool SimpleWindow::eventFilter(QObject *obj, QEvent *evt) {
	...
	return Window::SimpleWindow::eventFilter(obj, evt);
}
